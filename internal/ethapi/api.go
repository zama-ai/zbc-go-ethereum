...
func (s *BlockChainAPI) Call(ctx context.Context, args TransactionArgs, blockNrOrHash *rpc.BlockNumberOrHash, overrides *StateOverride, blockOverrides *BlockOverrides) (hexutil.Bytes, error) {
	...
	isEthCall := true
	result, err := DoCall(ctx, s.b, args, *blockNrOrHash, overrides, blockOverrides, s.b.RPCEVMTimeout(), s.b.RPCGasCap(), isGasEstimation, isEthCall)
	if err != nil {
		return nil, err
	}
...
func executeEstimate(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, gasCap uint64, gasLimit uint64) (bool, *core.ExecutionResult, error) {
	...
	isEthCall := false
	result, err := doCall(ctx, b, args, state, header, nil, nil, 0, gasCap, isGasEstimation, isEthCall)
	if err != nil {
