...
func (b *Block) Call(ctx context.Context, args struct {
	...
	isEthCall := true
	result, err := ethapi.DoCall(ctx, b.r.backend, args.Data, *b.numberOrHash, nil, nil, b.r.backend.RPCEVMTimeout(), b.r.backend.RPCGasCap(), isGasEstimation, isEthCall)
	if err != nil {
		return nil, err
	}
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	isEthCall := true
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, err := ethapi.DoCall(ctx, p.r.backend, args.Data, pendingBlockNr, nil, nil, p.r.backend.RPCEVMTimeout(), p.r.backend.RPCGasCap(), isGasEstimation, isEthCall)
	if err != nil {
		return nil, err
	}
	status := hexutil.Uint64(1)
